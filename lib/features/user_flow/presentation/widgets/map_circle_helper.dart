...
// --- 1. 検索範囲用のシンプルな青い円 (利用者用) ---
Set<Circle> createSearchRadiusWithSonar({
  required LatLng? center,
  required double radiusKm,
  double animationValue = 0, // 互換性のために維持
}) {
  if (center == null) return {};
...
Set<Circle> createActivePinSonar({
  ...
  // 固定の半径（300m）
  const double staticRadiusMeters = 300.0;

  return {
    Circle(
      circleId: CircleId('activeStaticCircle_$eventId'),
      center: center,
      radius: staticRadiusMeters,
      fillColor: Colors.greenAccent.shade700.withOpacity(0.15),
      strokeColor: Colors.greenAccent.shade700.withOpacity(0.4),
      strokeWidth: 1,
    ),
  };
}
